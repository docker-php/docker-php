...
class PluginResource extends Resource
{
     ...
     *
     * @return \Psr\Http\Message\ResponseInterface|\Docker\API\V1_27\Model\Plugin[]|\Docker\API\V1_27\Model\ErrorResponse
     */
    public function pluginList($parameters = [], $fetch = self::FETCH_OBJECT)
    {
        ...
        if (self::FETCH_OBJECT == $fetch) {
            if ('200' == $response->getStatusCode()) {
                return $this->serializer->deserialize((string) $response->getBody(), 'Docker\\API\\V1_27\\Model\\Plugin[]', 'json');
            }
     ...
     * @param string $fetch Fetch mode (object or response)
     *
     * @return \Psr\Http\Message\ResponseInterface|\Docker\API\V1_27\Model\PluginsPrivilegesGetResponse200Item[]|\Docker\API\V1_27\Model\ErrorResponse
     */
    public function getPluginPrivileges($parameters = [], $fetch = self::FETCH_OBJECT)
    {
        $queryParam = new QueryParam();
        $queryParam->setRequired('remote');
        $url     = '/v1.27/plugins/privileges';
        $url     = $url . ('?' . $queryParam->buildQueryString($parameters));
        ...
        if (self::FETCH_OBJECT == $fetch) {
            if ('200' == $response->getStatusCode()) {
                return $this->serializer->deserialize((string) $response->getBody(), 'Docker\\API\\V1_27\\Model\\PluginsPrivilegesGetResponse200Item[]', 'json');
            }
            if ('500' == $response->getStatusCode()) {
                return $this->serializer->deserialize((string) $response->getBody(), 'Docker\\API\\V1_27\\Model\\ErrorResponse', 'json');
            }
    ...
    {
        $queryParam = new QueryParam();
        $url        = '/v1.27/plugins/{name}/disable';
        $url        = str_replace('{name}', urlencode($name), $url);
        $url        = $url . ('?' . $queryParam->buildQueryString($parameters));
        $headers    = array_merge(['Host' => 'localhost'], $queryParam->buildHeaders($parameters));
        $body       = $queryParam->buildFormDataString($parameters);
    ...
    public function pluginUpgrade($name, array $body, $parameters = [], $fetch = self::FETCH_OBJECT)
    {
        ...
        $queryParam->setDefault('X-Registry-Auth', null);
        $queryParam->setHeaderParameters(['X-Registry-Auth']);
        $url     = '/v1.27/plugins/{name}/upgrade';
        $url     = str_replace('{name}', urlencode($name), $url);
        $url     = $url . ('?' . $queryParam->buildQueryString($parameters));
        $headers = array_merge(['Host' => 'localhost'], $queryParam->buildHeaders($parameters));
        $body    = $body;
