...
class PluginResource extends Resource
{
     ...
     *
     * @return \Psr\Http\Message\ResponseInterface|\Docker\API\V1_28\Model\Plugin[]|\Docker\API\V1_28\Model\ErrorResponse
     */
    public function pluginList($parameters = [], $fetch = self::FETCH_OBJECT)
    {
        ...
        if (self::FETCH_OBJECT == $fetch) {
            if ('200' == $response->getStatusCode()) {
                return $this->serializer->deserialize((string) $response->getBody(), 'Docker\\API\\V1_28\\Model\\Plugin[]', 'json');
            }
    ...
    {
        $queryParam = new QueryParam();
        $url        = '/v1.28/plugins/{name}/disable';
        $url        = str_replace('{name}', urlencode($name), $url);
        $url        = $url . ('?' . $queryParam->buildQueryString($parameters));
        $headers    = array_merge(['Host' => 'localhost'], $queryParam->buildHeaders($parameters));
        $body       = $queryParam->buildFormDataString($parameters);
    ...
    public function pluginUpgrade($name, array $body, $parameters = [], $fetch = self::FETCH_OBJECT)
    {
        ...
        $queryParam->setDefault('X-Registry-Auth', null);
        $queryParam->setHeaderParameters(['X-Registry-Auth']);
        $url     = '/v1.28/plugins/{name}/upgrade';
        $url     = str_replace('{name}', urlencode($name), $url);
        $url     = $url . ('?' . $queryParam->buildQueryString($parameters));
        $headers = array_merge(['Host' => 'localhost'], $queryParam->buildHeaders($parameters));
        $body    = $body;
